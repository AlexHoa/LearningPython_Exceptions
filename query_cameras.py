...
def parse(File):
	...
	try:
		...
		print('the file should contain name and extension')
	return T
...
''' Debut d'utilisation de map : '''
'''for i in T:
	t1 = (tuple(map(lambda x: x if 'one' in i else None, i)),)
	print(t1)
filtre_route('E40') = function map+filter+lambda
filtre_ville('Gent') = function map+filter+lambda
'''
''' first example to do with 'snelheidscameras_met_lus_120601.csv' '''
